...
void SingleLinkedList<T>::setCurrentVal(const T &_val)
{
    ...
    else throw std::runtime_error("Empty current position! Can't set value!");
}
/// 如果链表为空，返回 true；否则返回 false
template <typename T>
...
{
    if(currentPos != nullptr && currentPos->next != nullptr)    //currenPos下一个不为空
    {
        if(size == 1) emptyList();    //如果只有一个元素则删除此元素成为空列表
        else
        {
